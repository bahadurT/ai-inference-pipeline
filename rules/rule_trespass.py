...
class RuleTrespass:
    def __init__(self):
        ...
        self.trespass = {}
        self.attire_rule_executor = AttireRule()

    def execute_trespass_rules(self, camera,detection_map):
        try:
            ...
            for rule_name, categories in camera_rule.rule_details.items():
                ...
                for category, details_list in categories.items():
                    ...
                    if category == Constants.PERSON_DETECTION:
                        ...
                        print(f"    Category: {category}------------ executed sucessfully ------------>",camera.getCameraId())
                    elif category == Constants.ATTIRE_DETECTION:
                        print(f"    Category: {category}------------ executed sucessfully ----------------------------------------------------------------->",camera.getCameraId())
                        self.attire_rule_executor.execute_attire_rules(camera,detection_map,category, details_list)
