...
class KalmanFilterXYSR(object):
    ...
    def apply_affine_correction(self, m, t):
        """
        Apply affine correction to both the last state and last observation 
        for out-of-sample (OOS) smoothing.

        Handles internal logic for Kalman filter updates.
        """

        # Ensure matrix dimensions are correct
        scale = np.linalg.norm(m[:, 0])

        # Apply affine correction to the state vector (self.x) and covariance matrix (self.P)
        ...
        # If the object has not been observed and attr_saved is not None, apply correction to saved attributes
        if not self.observed and self.attr_saved is not None:
            if "x" in self.attr_saved:
                self.attr_saved["x"][:2] = m @ self.attr_saved["x"][:2] + t
                self.attr_saved["x"][4:6] = m @ self.attr_saved["x"][4:6]
            else:
                print("Warning: 'x' key is missing in attr_saved.")

            if "P" in self.attr_saved:
                self.attr_saved["P"][:2, :2] = m @ self.attr_saved["P"][:2, :2] @ m.T
                self.attr_saved["P"][4:6, 4:6] = m @ self.attr_saved["P"][4:6, 4:6] @ m.T
            else:
                print("Warning: 'P' key is missing in attr_saved.")

            # Ensure last_measurement is initialized properly before accessing
            if "last_measurement" in self.attr_saved and self.attr_saved["last_measurement"] is not None:
                self.attr_saved["last_measurement"][:2] = m @ self.attr_saved["last_measurement"][:2] + t
            else:
                print("Warning: 'last_measurement' is either missing or None in attr_saved.")

    def old_apply_affine_correction(self, m, t):
        """
