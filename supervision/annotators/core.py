...
class Dim3ColorAnnotator(BaseAnnotator):
    """
    A class for drawing transparent 3D bounding boxes (cuboids) on an image using provided detections.
    """
    ...
    def __init__(
        ...
        depth_offset: int = 20,  # Depth offset for the 3D effect
        alpha: float = 0.5  # Transparency level (0: fully transparent, 1: fully opaque)
    ):
        """
        Args:
            ...
            depth_offset (int): Offset for creating the 3D cuboid effect.
            alpha (float): Transparency level of the cuboid (0: fully transparent, 1: fully opaque).
        """
        self.color: Union[Color, ColorPalette] = color
        self.thickness: int = thickness
        self.color_lookup: ColorLookup = color_lookup
        self.depth_offset: int = depth_offset  # How far the second box will be offset for 3D look
        self.alpha: float = alpha  # Transparency
    ...
    def annotate(
        ...
        """
        Annotates the given scene with transparent 3D bounding boxes (cuboids) based on the provided detections.
        """
        assert isinstance(scene, np.ndarray)

        # Create an overlay image to draw the transparent cuboids
        overlay = scene.copy()
        ...
        for detection_idx in range(len(detections)):
            ...
            # Front face (original bounding box)
            cv2.rectangle(
                img=overlay,
                pt1=(x1, y1),
                pt2=(x2, y2),
                color=bgr_color,
                thickness=cv2.FILLED,  # Fill the front face
            )

            # Calculate the offset to create the "depth" of the cuboid
            x1_offset, y1_offset = x1 - self.depth_offset, y1 - self.depth_offset
            x2_offset, y2_offset = x2 - self.depth_offset, y2 - self.depth_offset

            # Back face (offset bounding box)
            cv2.rectangle(
                img=overlay,
                pt1=(x1_offset, y1_offset),
                pt2=(x2_offset, y2_offset),
                color=bgr_color,
                thickness=cv2.FILLED,  # Fill the back face
            )

            # Draw the connecting lines between the front and back faces
            cv2.line(overlay, (x1, y1), (x1_offset, y1_offset), bgr_color, self.thickness)  # Top-left
            cv2.line(overlay, (x2, y1), (x2_offset, y1_offset), bgr_color, self.thickness)  # Top-right
            cv2.line(overlay, (x1, y2), (x1_offset, y2_offset), bgr_color, self.thickness)  # Bottom-left
            cv2.line(overlay, (x2, y2), (x2_offset, y2_offset), bgr_color, self.thickness)  # Bottom-right

            # Fill the sides
            # Left side
            cv2.fillPoly(overlay, np.array([[ 
                (x1, y1), (x1, y2), 
                (x1_offset, y2_offset), (x1_offset, y1_offset)
            ]]), bgr_color)

            # Right side
            cv2.fillPoly(overlay, np.array([[ 
                (x2, y1), (x2, y2), 
                (x2_offset, y2_offset), (x2_offset, y1_offset)
            ]]), bgr_color)

            # Top side
            cv2.fillPoly(overlay, np.array([[ 
                (x1, y1), (x2, y1), 
                (x2_offset, y1_offset), (x1_offset, y1_offset)
            ]]), bgr_color)

        # Blend the overlay with the original image to apply transparency
        cv2.addWeighted(overlay, self.alpha, scene, 1 - self.alpha, 0, scene)

        return scene



class Dim3BoxAnnotator(BaseAnnotator):
    """
    A class for drawing 3D bounding boxes (cuboids) on an image using provided detections.
    """
    ...
    def annotate(
        ...
        """
        Annotates the given scene with 3D bounding boxes (cuboids) based on the provided detections.
        """
        assert isinstance(scene, np.ndarray)
        ...
        for detection_idx in range(len(detections)):
            ...
            # Connecting lines between the front and back face
            cv2.line(scene, (x1, y1), (x1_offset, y1_offset), color.as_bgr(), self.thickness)
            cv2.line(scene, (x2, y1), (x2_offset, y1_offset), color.as_bgr(), self.thickness)
            cv2.line(scene, (x1, y2), (x1_offset, y2_offset), color.as_bgr(), self.thickness)
            cv2.line(scene, (x2, y2), (x2_offset, y2_offset), color.as_bgr(), self.thickness)
...
class EllipseAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        """
        Annotates the given scene with transparent filled ellipses based on the provided detections.
        """
        assert isinstance(scene, np.ndarray)

        # Create an overlay with the same dimensions as the scene, but with 4 channels (RGBA)
        overlay = scene.copy()
        ...
        for detection_idx in range(len(detections)):
            ...
            # Draw the filled ellipse on the overlay
            cv2.ellipse(
                overlay,
                center=center,
                ...
                color=color.as_bgr(),  # Use the BGR color
                thickness=-1,  # Filled ellipse
                lineType=cv2.LINE_4,
            )

        # Blend the overlay with the original scene using alpha transparency
